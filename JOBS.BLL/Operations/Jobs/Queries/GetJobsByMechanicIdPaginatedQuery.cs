...
{
    public class GetJobsByMechanicIdPaginatedQuery : PaginationParams, IRequest<Pagination<JobDTO>>
    {
    ...
    public class GetJobsByMechanicIdPaginatedQueryHandler : IRequestHandler<GetJobsByMechanicIdPaginatedQuery, Pagination<JobDTO>>
    {
        private readonly ServiceStationDBContext _context;
        private readonly IMapper _mapper;

        public GetJobsByMechanicIdPaginatedQueryHandler(ServiceStationDBContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }

        public async Task<Pagination<JobDTO>> Handle(GetJobsByMechanicIdPaginatedQuery request, CancellationToken cancellationToken)
        {
            var query = _context.Jobs.Where(j => j.MechanicId == request.MechanicId);
